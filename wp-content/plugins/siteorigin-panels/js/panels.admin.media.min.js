...
originalInsert(h);}
$('body').on({click:function(event){if(typeof wp==='undefined'||!wp.media||!wp.media.gallery)return;event.preventDefault();var $$=$(this);var dialog=$('.panels-admin-dialog:visible');var val=dialog.find('*[name$="[ids]"]').val();if(val.indexOf('{demo')===0||val.indexOf('{default')===0)val='-';if(val==''&&$('#post_ID').val()==null)val='-';var frame=wp.media.gallery.edit('[gallery ids="'+val+'"]');frame.state('gallery-edit').on('update',function(selection){var ids=selection.models.map(function(e){return e.id});dialog.find('input[name$="[ids]"]').val(ids.join(','));});frame.on('escape',function(){dialog.find('.ui-dialog-content').dialog('open');});return false;}},'.so-gallery-widget-select-attachments');});